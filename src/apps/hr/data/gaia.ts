...
export type GaiaSortBy = "brightest" | "closest" | "random";
...
export async function queryConeSearch(
  ...
  const sortBy = options.sortBy ?? "closest";
  // For non-brightest sort orders, fetch a larger pool so the
  // client-side reordering has something to work with. Brightest
  // mode goes straight at topN to minimise catalog load.
  const fetchN = sortBy === "brightest"
    ? topN
    : Math.min(1000, Math.max(topN * 3, topN + 50));
  ...
  let rows = await runAdql(adql, options.signal);
  rows = sortGaiaRows(rows, sortBy, raDeg, decDeg).slice(0, topN);
  await enrichWithParamsup(rows, options.signal);
  return rows;
}

function sortGaiaRows(
  rows: GaiaRow[],
  sortBy: GaiaSortBy,
  centerRaDeg: number,
  centerDecDeg: number,
  ...
  if (sortBy === "brightest") {
    return rows.slice().sort((a, b) => a.g_mag - b.g_mag);
  }
  if (sortBy === "random") {
  ...
  // "closest" — angular distance from the reticle on the sky
  // (great-circle, in degrees).
  ...
  return rows
    .slice()
    .map((r) => {
      const ra = (r.ra * Math.PI) / 180;
      const dec = (r.dec * Math.PI) / 180;
